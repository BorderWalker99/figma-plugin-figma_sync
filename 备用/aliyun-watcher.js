...
async function initializeKnownFiles() {
  ...
  try {
    ...
    if (allFiles.length > 0 && ws && ws.readyState === WebSocket.OPEN) {
      try {
        ws.send(JSON.stringify({
          type: 'toast',
          message: `实时模式已启动 (忽略 ${allFiles.length} 个现有文件，如需同步请使用"手动同步")`,
          duration: 5000,
          level: 'info'
        }));
      } catch (e) {
...
async function countFilesForManualSync() {
  ...
  if (!CONFIG.userFolderId) {
    console.log('⚠️  [OSS] 用户文件夹未初始化');
    return;
  }
  
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    console.error('❌ [OSS] WebSocket 未连接');
    return;
  }
  ...
  try {
    ...
    console.error('❌ [OSS] 统计文件失败:', error.message);
  }
}
