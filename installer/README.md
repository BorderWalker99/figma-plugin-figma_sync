...
5. **配置设置**：自动配置用户配置和本地文件夹
6. **完成安装**：预热运行时权限、验证关键二进制并启动服务器
...
- 安装器会在最后一步提前验证内置 `node`、`ffmpeg`、`gifsicle`、`ImageMagick`，尽量把 Gatekeeper 问题前置到安装阶段。
- 如果系统仍拦截个别运行时工具，安装器会先引导用户完成「仍要打开」授权，再继续后续步骤。

## 注意事项

- 建议使用 macOS 11 或更高版本
- 某些步骤需要管理员权限（会弹出密码输入框）
- macOS 11-13 首次安装 Homebrew 可能较慢
