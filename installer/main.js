...
async function repairSharpRuntimeForCurrentArch({ installPath, npmPath, nodePath, sendOutput }) {
  ...
  try {
    ...
    for (const registry of registries) {
      try {
        log(`   ↪️ 正在下载并安装 sharp（registry=${registry}）...\n`);
        await execPromise(
          `"${npmPath}" --prefix "${installPath}" install --no-save --include=optional --legacy-peer-deps --os=darwin --cpu=${cpu} sharp --no-audit --no-fund --registry=${registry}`,
    ...
    if (lastInstallErr) {
      try {
        ...
        await execPromise(
          `"${npmPath}" --prefix "${installPath}" rebuild sharp --include=optional --os=darwin --cpu=${cpu} --no-audit --no-fund`,
          { timeout: 6 * 60 * 1000, env: { ...process.env } }
        );
      } catch (__ ) {
        // continue to offline restore below if available
      }
    }
  } catch (_) {
    // ignore and rely on health check + offline restore below
  }
...
ipcMain.handle('install-dependencies', async (event, installPath) => {
  return new Promise(async (resolve) => {
    ...
    // Prefer bundled Node + npm (ensures arch match for native addons like sharp)
    if (nodeBinForHealth) {
    ...
    // 明确包含 optionalDependencies（sharp 的预编译包依赖它）
    const commandStr = `"${npmPath}" install --legacy-peer-deps --omit=dev --include=optional --no-audit --no-fund --registry=https://registry.npmmirror.com --prefix "${installPath}"`;
