...
function getManualSyncVideoTimeoutMs(fileSizeMB = 0) {
  const safeSizeMB = Math.max(0, Number(fileSizeMB) || 0);
  const pressure = getSystemPressure(mediaTuning);
  const runtimeCfg = mediaTuning.adaptiveRuntime || {};
  const lowCoreCount = Math.max(2, Math.round(Number.isFinite(runtimeCfg.lowCoreCount) ? runtimeCfg.lowCoreCount : 4));
  let timeoutMs = getAdaptiveVideoTimeoutMs(mediaTuning, { fileSizeMB: safeSizeMB, pressure });

  // 手动同步的视频超时覆盖了下载、预缩放、GIF 转码和回传。
  // 老机器在低负载下不一定会提早进入极速档，因此这里额外放宽预算。
  if (safeSizeMB >= 60) timeoutMs = Math.max(timeoutMs, 12 * 60 * 1000);
  if (safeSizeMB >= 90) timeoutMs = Math.max(timeoutMs, 16 * 60 * 1000);
  if (safeSizeMB >= 150) timeoutMs = Math.max(timeoutMs, 20 * 60 * 1000);

  if (pressure.cpuCount <= lowCoreCount) {
    timeoutMs += safeSizeMB >= 90 ? 2 * 60 * 1000 : 90 * 1000;
  }

  return Math.min(30 * 60 * 1000, timeoutMs);
}
...
const realtimeProcessingFileIds = new Set(); // 实时模式当前正在处理/转换中的文件 ID，手动同步需要提前跳过
const realtimeImageQueue = [];
const realtimeQueuedImageFileIds = new Set();
const realtimeVideoQueue = [];
const realtimeQueuedVideoFileIds = new Set();
const realtimeRetryTimers = new Set();
let isDriveRealtimeImageQueueRunning = false;
let isDriveRealtimeVideoQueueRunning = false;
let isClearingUserCloudFiles = false;
...
const REALTIME_FILE_MAX_RETRIES = 4;
...
function getDriveRealtimeSortTime(file) {
  return Math.max(
    0,
    new Date(file && (file.createdTime || file.modifiedTime) || 0).getTime() || 0
  );
}

function compareDriveRealtimeFilesByTimestamp(a, b) {
  const timeDiff = getDriveRealtimeSortTime(a) - getDriveRealtimeSortTime(b);
  if (timeDiff !== 0) return timeDiff;
  return String((a && a.id) || '').localeCompare(String((b && b.id) || ''));
}

function insertDriveRealtimeFileInOrder(queue, file) {
  if (!Array.isArray(queue) || !file) return;
  let index = queue.length;
  while (index > 0 && compareDriveRealtimeFilesByTimestamp(file, queue[index - 1]) < 0) {
    index -= 1;
  }
  queue.splice(index, 0, file);
}

function enqueueDriveRealtimeImage(file) {
  if (!file || !file.id) return;
  if (realtimeQueuedImageFileIds.has(file.id)) return;
  realtimeQueuedImageFileIds.add(file.id);
  insertDriveRealtimeFileInOrder(realtimeImageQueue, file);
  drainDriveRealtimeImageQueue().catch(() => {});
}
...
function enqueueDriveRealtimeVideo(file) {
  ...
  insertDriveRealtimeFileInOrder(realtimeVideoQueue, file);
  drainDriveRealtimeVideoQueue().catch(() => {});
}

function scheduleDriveRealtimeRetry(file, reason = 'retry') {
  if (!file || !file.id || !isRealTimeMode) return;
  const nextAttempt = Number(file._realtimeAttempt || 0) + 1;
  if (nextAttempt > REALTIME_FILE_MAX_RETRIES) {
    console.warn(`⚠️  [Drive] 实时重试上限，放弃: ${file.name} (${reason})`);
    return;
  }

  const retryFile = { ...file, _realtimeAttempt: nextAttempt };
  const delayMs = REALTIME_RETRY_BASE_DELAY_MS * Math.min(nextAttempt, 5);
  const timer = setTimeout(async () => {
    realtimeRetryTimers.delete(timer);
    if (!isRealTimeMode) return;

    const fName = (retryFile.name || '').toLowerCase();
    const fMime = (retryFile.mimeType || '').toLowerCase();
    const isVideoFile = fName.endsWith('.mp4') || fName.endsWith('.mov') || fMime.startsWith('video/');

    if (isVideoFile) {
      enqueueDriveRealtimeVideo(retryFile);
      return;
    }
    enqueueDriveRealtimeImage(retryFile);
  }, delayMs);
  ...
  try {
    while (realtimeVideoQueue.length > 0 && isRealTimeMode) {
      if (realtimeImageQueue.length > 0) {
        await drainDriveRealtimeImageQueue();
        continue;
      }
      const file = realtimeVideoQueue.shift();
      if (!file) continue;
      realtimeQueuedVideoFileIds.delete(file.id);
      const fileSizeMB = Math.max(0, Number(file.size || 0) / 1024 / 1024);
      const videoTimeoutMs = getAdaptiveVideoTimeoutMs(mediaTuning, { fileSizeMB });
      let _timeoutId;
      const fileTimeout = new Promise((_, reject) => {
        _timeoutId = setTimeout(() => reject(new Error(`SYNC_TIMEOUT:${file.name}`)), videoTimeoutMs);
      });
      try {
        ...
        scheduleDriveRealtimeRetry(file, msg);
      } finally {
...
async function drainDriveRealtimeImageQueue() {
  if (isDriveRealtimeImageQueueRunning) return;
  isDriveRealtimeImageQueueRunning = true;
  try {
    const IMAGE_TIMEOUT_MS = 60000;
    while (realtimeImageQueue.length > 0 && isRealTimeMode) {
      const file = realtimeImageQueue.shift();
      if (!file) continue;
      realtimeQueuedImageFileIds.delete(file.id);

      let _timeoutId;
      const fileTimeout = new Promise((_, reject) => {
        _timeoutId = setTimeout(() => reject(new Error(`SYNC_TIMEOUT:${file.name}`)), IMAGE_TIMEOUT_MS);
      });
      ...
      try {
        ...
        if (isTimeout) {
          safeSend({
            ...
            filenames: [file.name],
            failures: [{ filename: file.name, reasonCode: 'network', detail: 'sync-timeout' }]
          });
          console.warn(`   ⚠️  [大文件] 下载/处理超时，保留云端文件避免丢失: ${file.name}`);
        } else {
          console.error(`   ❌ 实时图片处理失败: ${file.name} ${msg}`);
          ...
          if (reason.code !== 'unknown') {
            safeSend({
              ...
              filenames: [file.name],
              failures: [{ filename: file.name, reasonCode: reason.code, detail: reason.detail }]
            });
          }
        }
        scheduleDriveRealtimeRetry(file, msg);
    ...
    isDriveRealtimeImageQueueRunning = false;
    if (realtimeImageQueue.length > 0 && isRealTimeMode) {
      drainDriveRealtimeImageQueue().catch(() => {});
    }
  }
}
...
function cleanupFileRecord(fileId, md5Checksum = null) {
  ...
  realtimeProcessingFileIds.delete(fileId);
...
async function pollDrive() {
  ...
  try {
    ...
    if (newFiles.length > 0) {
      console.log(`🔄 [Drive] 检测到 ${newFiles.length} 个新文件，按时间顺序处理...`);
      
      // 分类：图片进入图片优先队列，录屏进入视频队列；两类内部都按时间顺序处理。
      const imageFiles = [];
      const videoFiles = [];
      ...
      try {
        if (imageFiles.length > 0) {
          for (const imageFile of imageFiles) {
            enqueueDriveRealtimeImage(imageFile);
          }
        }
        if (videoFiles.length > 0) {
...
async function handleDriveFile(file, deleteAfterSync = false, progressCb = null, shouldAbort = null, syncSource = 'realtime') {
  ...
  if (syncSource === 'realtime') {
    realtimeProcessingFileIds.add(file.id);
  }
  let videoTempDir = null;
  ...
  try {
    ...
    const recordingTaskId = looksLikeVideo
      ? recordingTaskStore.createTask({
          source: 'drive',
          syncSource,
          stage: 'queued',
    ...
    if (recordingTaskId) {
      recordingTaskStore.updateStage(recordingTaskId, 'downloading', { progress: 5 });
      notifyRecordingTaskUpdate(recordingTaskId, { stage: 'downloading' });
    }
    let downloadProgressTimer = null;
    let lastDownloadPct = 5;
    const expectedDownloadBytes = Math.max(0, Number(file.size || 0));
    if (looksLikeVideo) {
      // 没有文件大小时退回到伪进度；有大小时优先使用真实下载字节进度。
      if (!expectedDownloadBytes) {
        let downloadingPct = 5;
        downloadProgressTimer = setInterval(() => {
          try {
            downloadingPct = Math.min(19, downloadingPct + 1);
            lastDownloadPct = downloadingPct;
    ...
    try {
      if (looksLikeVideo) {
        const downloadVideoExt =
          fileNameLower.endsWith('.mov') || fileMimeLower === 'video/quicktime' ? '.mov' : '.mp4';
        videoTempDir = path.join(os.tmpdir(), `screensync-drive-src-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
        fs.mkdirSync(videoTempDir, { recursive: true });
        downloadedVideoPath = path.join(videoTempDir, `input${downloadVideoExt}`);
        await downloadFileToPath(file.id, downloadedVideoPath, downloadTimeout, 3, {
          expectedBytes: expectedDownloadBytes,
          onProgress: ({ downloadedBytes, totalBytes }) => {
            if (!totalBytes || totalBytes <= 0) return;
            const ratio = Math.max(0, Math.min(1, downloadedBytes / totalBytes));
            const nextPct = Math.max(lastDownloadPct, Math.min(27, 5 + Math.round(ratio * 22)));
            if (nextPct > lastDownloadPct) {
              lastDownloadPct = nextPct;
              emitProgress('downloading', nextPct, {
                isVideo: true,
                stageDetail: 'downloading-large-video',
                downloadedBytes,
                totalBytes
              });
            }
        ...
        originalBuffer = await downloadFileBuffer(file.id, downloadTimeout, 3, {
          expectedBytes: expectedDownloadBytes,
          onProgress: null
        });
      }
    ...
    throwIfAborted();
    const downloadedSizeBytes = looksLikeVideo
      ? (fs.existsSync(downloadedVideoPath) ? fs.statSync(downloadedVideoPath).size : 0)
      : (originalBuffer ? originalBuffer.length : 0);
    const downloadedSizeKB = (downloadedSizeBytes / 1024).toFixed(2);
    emitProgress('downloading', looksLikeVideo ? 27 : 20, { sizeKB: parseFloat(downloadedSizeKB), ...(looksLikeVideo ? { isVideo: true } : {}) });
    if (recordingTaskId) {
      recordingTaskStore.updateStage(recordingTaskId, 'downloaded', {
        progress: 27,
        downloadedSizeBytes,
        sourcePath: downloadedVideoPath || null
      });
      notifyRecordingTaskUpdate(recordingTaskId, { stage: 'downloaded' });
    }
    ...
    if (isVideo) {
      const videoSizeBytes = downloadedSizeBytes;
      ...
      const sourceMeta = await ffprobeVideoMeta(tempVideoPath);
      ...
      console.log(`   🎬 [Video→GIF] 开始共享转码 ${file.name} (${videoSizeMB}MB, pixels=${sourcePixels}, frames=${sourceFrames || '?'})...`);
      emitProgress('converting', 25, { estimatedSec, isVideo: true, stageDetail: 'shared-gif-pipeline' });
      if (recordingTaskId) {
        recordingTaskStore.updateStage(recordingTaskId, 'converting', {
          progress: 25,
          estimatedSec,
          sourceVideoPath: tempVideoPath,
          sourceVideoSizeBytes: videoSizeBytes
        });
      ...
      try {
        ...
        await transcodeVideoToGif({
          ...
          shouldCancel: () => _abortAllConversions || (typeof shouldAbort === 'function' && shouldAbort()),
          onProgress: (percent, extra = {}) => {
            emitProgress('converting', percent, {
              estimatedSec,
              isVideo: true,
        ...
        try {
          const gifCacheResult = userConfig.saveGifFileToCache(tempGifOut, file.name, file.id);
          if (gifCacheResult) gifCacheId = gifCacheResult.cacheId;
        } catch (_) {}
        const gifDims = parseGifDimensionsFromFile(tempGifOut);
        if (recordingTaskId) {
        ...
        if (backupMode === 'gif_only' || backupMode === 'all') {
          if (gifSizeBytes > LARGE_GIF_URL_THRESHOLD) {
            ...
            const sr = await saveFileToLocalFolder(gifBuffer, file.name, 'image/gif');
        ...
        console.error(`   ❌ [Video→GIF] 共享链路失败 (${((Date.now() - convStartTime) / 1000).toFixed(1)}秒): ${convErr.message}`);
        if (recordingTaskId) {
          recordingTaskStore.updateStage(recordingTaskId, 'failed', {
            progress: 100,
            lastError: convErr.message || String(convErr)
          });
    ...
    if (conversionOk) {
      ...
      console.log(`   🗂️ [Drive同步] 录屏任务已转为后台导入任务: ${file.name}${recordingTaskId ? ` (${recordingTaskId})` : ''}`);
    ...
    if (videoTempDir) {
      try { fs.rmSync(videoTempDir, { recursive: true, force: true }); } catch (_) {}
    }
    processingFileIds.delete(file.id);
    realtimeProcessingFileIds.delete(file.id);
  }
}
...
async function countFilesForManualSync() {
  ...
  try {
    ...
    const listAllFolderFiles = async ({ folderId, pageSize = 1000, orderBy = 'createdTime asc', fields = 'files(id,name,mimeType,createdTime,modifiedTime,size,parents,md5Checksum),nextPageToken' }) => {
      let pageToken = null;
      const allFiles = [];
      do {
        const { files, nextPageToken } = await listFolderFiles({
          folderId,
          pageSize,
          orderBy,
          fields,
    ...
    const listPromise = listAllFolderFiles({ 
      folderId: CONFIG.userFolderId, 
      pageSize: 1000, 
    ...
    const imageFiles = files.filter(file => {
      ...
      return mimeType.startsWith('image/') || mimeType.startsWith('video/') ||
             /\.(jpg|jpeg|png|gif|webp|heic|heif|mp4|mov)$/i.test(name);
    }).filter(file => !realtimeProcessingFileIds.has(file.id));
...
async function clearUserCloudFiles() {
  if (isClearingUserCloudFiles) {
    safeSend({
      type: 'clear-user-cloud-files-done',
      success: false,
      count: 0,
      message: '云文件清理正在进行中'
    });
  ...
  const isBusy =
    ...
    realtimeProcessingFileIds.size > 0 ||
    realtimeImageQueue.length > 0 ||
    realtimeVideoQueue.length > 0 ||
    isDriveRealtimeImageQueueRunning ||
    isDriveRealtimeVideoQueueRunning;
  ...
  if (isBusy) {
    safeSend({
      ...
      success: false,
      count: 0,
      message: '当前有同步任务进行中，请稍后重试'
    });
    return;
  }

  isClearingUserCloudFiles = true;
  let clearedCount = 0;
  ...
  try {
    ...
    const files = [];
    do {
      const { files: pageFiles, nextPageToken } = await listFolderFiles({
        folderId: CONFIG.userFolderId,
        pageSize: 1000,
        orderBy: 'createdTime asc',
    ...
    for (const file of files) {
      ...
      await deleteDriveSourceFileWithFallback(file.id, file.name || 'unknown', file.md5Checksum || null);
      clearedCount += 1;
    }
    ...
    isClearingUserCloudFiles = false;
  }
}
...
async function performManualSync() {
  ...
  const overallTimeout = new Promise((_, reject) => {
    _overallTimeoutId = setTimeout(() => reject(new Error('手动同步总体超时（超过30分钟），请检查网络连接或减少待同步文件数量')), 1800000);
  });
  ...
  const syncTask = (async () => {
    ...
    let skippedRealtimeProcessingCount = 0;
    ...
    for (const file of files) {
      ...
      if (realtimeProcessingFileIds.has(file.id)) {
        skippedRealtimeProcessingCount++;
        console.log(`   ⏭️  [手动同步] 跳过实时模式正在处理的文件: ${file.name} (${file.id})`);
        continue;
      }
    ...
    const orderedFiles = refreshedFiles.slice().sort((a, b) => {
      const diff = new Date(a.createdTime || 0).getTime() - new Date(b.createdTime || 0).getTime();
    ...
    // 分类统计
    const videoFileCount = orderedFiles.filter(f => {
    ...
    const fileOrderMap = new Map(orderedFiles.map((file, idx) => [file.id, idx + 1]));
    const manualPressure = getSystemPressure(mediaTuning);
    const hasVeryLargeVideo = videoBatch.some(file => Math.max(0, Number(file.size || 0) / 1024 / 1024) >= 90);
    ...
    const processOneFile = async (file, myIndex) => {
      ...
      const fMime = (file.mimeType || '').toLowerCase();
      const isVideoItem = fName.endsWith('.mp4') || fName.endsWith('.mov') || fMime.startsWith('video/');
      const timeoutMs = isVideoItem
        ? getManualSyncVideoTimeoutMs(Math.max(0, Number(file.size || 0) / 1024 / 1024))
        : 60000;
      ...
      const fileProgressCb = (stage, percent, extra = {}) => {
        ...
        const stableFilePct = Math.max(activeFileProgress.get(file.id) || 0, filePct);
        activeFileProgress.set(file.id, stableFilePct);
        sendProgress('manual-sync-progress', {
          total: totalFiles, completed: completedCount,
          percent: getAggregatedManualPercent(),
          fileIndex: myIndex, filename: file.name,
          filePercent: stableFilePct, stage,
          isVideo: extra.isVideo === true || isVideoItem
        });
      };
    ...
    let maxReportedManualPercent = 0;
    ...
    const getAggregatedManualPercent = () => {
      ...
      const nextPercent = Math.min(99, Math.max(0, Math.round(((completedCount + partialCompleted) / totalFiles) * 100)));
      maxReportedManualPercent = Math.max(maxReportedManualPercent, nextPercent);
      return maxReportedManualPercent;
    };

    const handleManualFileResult = (value, fileOrder, fileName) => {
      results.push({ status: 'fulfilled', value });
      if (value && value.file && value.file.id) {
        activeFileProgress.delete(value.file.id);
      }
    ...
    const runManualWorker = async (batch) => {
      for (const file of batch) {
        ...
        const fileOrder = fileOrderMap.get(file.id) || 0;
        const value = await processOneFile(file, fileOrder);
        if (value && value.cancelled) {
          if (value.file && value.file.id) activeFileProgress.delete(value.file.id);
          cancelled = true;
          break;
        }
    ...
    console.log(`   ℹ️  [手动同步] 图片优先，随后处理视频: ${manualPressure.label}${hasVeryLargeVideo ? ', very-large-video' : ''}`);
    await runManualWorker(imageBatch);
    if (!cancelled) {
      await runManualWorker(videoBatch);
    }
...
function stopPolling() {
  ...
  for (const timer of realtimeRetryTimers) {
...
function releaseRuntimeResources(options = {}) {
  ...
  stopPolling();
  processingFileIds.clear();
  realtimeProcessingFileIds.clear();
  realtimeImageQueue.length = 0;
  realtimeQueuedImageFileIds.clear();
  realtimeVideoQueue.length = 0;
  realtimeQueuedVideoFileIds.clear();
  isDriveRealtimeImageQueueRunning = false;
  isDriveRealtimeVideoQueueRunning = false;
...
function connectWebSocket() {
  ...
  ws.on('message', async (data) => {
    try {
      ...
      if (message.type === 'clear-user-cloud-files') {
        await clearUserCloudFiles();
        return;
      }
