...
if [ -f "安装前_将此文件拖进终端按回车运行.command" ]; then
    cp "安装前_将此文件拖进终端按回车运行.command" "$TEMP_DIR/"
    chmod +x "$TEMP_DIR/安装前_将此文件拖进终端按回车运行.command"
    echo "   ✅ Gatekeeper 修复脚本已包含（首层目录）"
fi

# 复制手动连接脚本（放在首层）
if [ -f "若连接断开_将此文件拖进终端按回车运行.command" ]; then
    cp "若连接断开_将此文件拖进终端按回车运行.command" "$TEMP_DIR/"
    chmod +x "$TEMP_DIR/若连接断开_将此文件拖进终端按回车运行.command"
    echo "   ✅ 手动连接脚本已包含（首层目录）"
fi
...
A: 这是 macOS 安全机制。解决方法：
   1. 将"安装前_将此文件拖进终端按回车运行.command"拖入终端窗口
   2. 按回车键运行
...
A: 这种情况极少见，但如果发生：
   ...
   方法2（备选）：使用手动启动脚本
   1. 将 "若连接断开_将此文件拖进终端按回车运行.command" 拖入终端
   2. 按回车键运行，选择相应的修复选项
...
2. 将文件"安装前_将此文件拖进终端按回车运行.command"拖入终端窗口
3. 按回车键运行
4. 看到"✅ 准备完成！"后关闭终端
...
echo -e "${BLUE}║     或将"若连接断开_将此文件..."拖入终端并回车             ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}\n"
