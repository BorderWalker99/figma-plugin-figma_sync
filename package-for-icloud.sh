...
for f in server.js userConfig.js start.js setup-autostart.js recording-task-store.js gif-composer.js video-gif-pipeline.js image-processor.js adaptive-processing.js icloud-watcher.js; do
    if [ -f "$f" ]; then
        cp "$f" "$PROJECT_DIR/"
        echo "   ✅ $f"
    fi
