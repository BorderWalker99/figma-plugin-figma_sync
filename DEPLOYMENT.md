...
指导用户使用 macOS 原生的"右键打开"功能绕过 Gatekeeper：

1. **右键点击** `ScreenSync Installer.app`
2. 选择 **打开**
3. 在弹出的安全警告窗口中，点击 **打开**

这只需要操作一次，之后就可以直接双击打开了。

**备选方法（命令行）**：

如果上述方法无效，可以使用终端移除隔离属性：
```bash
# 在终端中运行
...
### 问题 12：安装器中"安装 Homebrew"按钮点击后没反应

**症状**：
用户点击"安装 Homebrew"按钮后，没有任何反应，按钮显示"安装中..."但一直没有进展

**原因**：
之前的实现使用 `stdio: 'inherit'`，但 Electron GUI 应用没有关联的终端，导致：
- Homebrew 安装脚本需要的交互（输入密码、确认）无法显示
- 进程在后台等待用户输入，但用户看不到任何提示
- 看起来"卡住"或"没反应"

**已修复**：
✅ 现在会自动打开 macOS 终端应用
✅ 在终端中运行 Homebrew 安装脚本
✅ 用户可以看到安装进度和提示
✅ 安装完成后点击"重新检测"按钮即可继续
